...
@Component({
  selector: 'app-edit-and-hold-district',
  templateUrl: './edit-and-hold-district.component.html',
  styleUrls: ['./edit-and-hold-district.component.css']
})
export class EditAndHoldDistrictComponent implements OnInit {

  dataSource: any;
  displayedColumns: string[] = ['artistCode', 'firstName', 'lastName', 'approvalStatus', 'view'];
  ...
  getEditOrHoldArtistAtDistrict(userId) {
    this.employeeService.getEditOrHoldMaster(userId).subscribe(res => {
      this.personalDetailsData = res;
      let uniqueData = _.uniqBy(this.personalDetailsData, 'id');
      this.personalDetailsData = uniqueData;
      this.dataSource = new MatTableDataSource(this.personalDetailsData);
